        ...
        <AdminGuard>
            <div className="space-y-8">
                <div>
                    ...
                    <p className="text-gray-500 mt-1">Generá y administrá los códigos para tus clientes.</p>
                </div>
                    ...
                    <form onSubmit={handleCreate} className="flex flex-col sm:flex-row gap-3">
                        <div className="flex gap-2 flex-1">
                            ...
                            <input
                                ...
                                placeholder="Nota (ej: Bar El Patio - Juan)"
                                className="flex-1 px-3 py-2 border border-gray-300 rounded-md text-sm text-gray-700 focus:outline-none focus:ring-2 focus:ring-red-500"
                            />
                        </div>
                        <button
                            type="submit"
                ...
                <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
                    ...
                    {loading ? (
                        ...
                        <ul className="divide-y divide-gray-100">
                            {invitations.map(inv => (
                                <li key={inv.code} className="flex items-center gap-4 px-6 py-4">
                                    <span className={`inline-flex shrink-0 items-center px-2 py-0.5 rounded text-xs font-medium ${inv.used ? 'bg-gray-100 text-gray-500' : 'bg-green-100 text-green-700'}`}>
                                        {inv.used ? 'Usado' : 'Disponible'}
                                    </span>
                                    <span className="font-mono text-sm font-semibold text-gray-800 tracking-wider">{inv.code}</span>
                                    <span className="flex-1 text-sm text-gray-500 truncate">{inv.note || '—'}</span>
                                    {inv.used && inv.usedBy ? (
                                        <span className="text-xs text-gray-400 hidden sm:block truncate max-w-[180px]">{inv.usedBy}</span>
                                    ) : (
                                        <span className="text-xs text-gray-400 hidden sm:block">{formatDate(inv.createdAt)}</span>
                                    )}
